...
export const usePdfMetaStore = defineStore('pdfMeta', () => {
  const meta = ref<PdfMetadata>(createEmptyPdfMeta())
  const autoMeta = ref<PdfMetadata>(createEmptyPdfMeta())
  let synced = false
...
function shouldSyncField(currentValue: string, previousAutoValue: string): boolean {
