export function buildPersonalMetrics(personal, rawText) {
    ...
    const score = Math.min(1, (value.fullName.trim() ? 0.45 : 0) +
...
export function toConfidenceLevel(metrics) {
    if (!metrics.rawText.trim() && metrics.extractedCount === 0)
        return 'missing';
...
export function buildConfidenceNote(key, metrics, confidence, t) {
