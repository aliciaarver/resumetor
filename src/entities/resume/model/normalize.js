...
function composeFullName(personal) {
    const parts = [
...
export function normalizeResumeData(value) {
    ...
    const workFormats = Array.isArray(personal.workFormats)
        ? personal.workFormats.filter((entry) => typeof entry === 'string').map((entry) => entry.trim()).filter(Boolean)
        : [];
    const workExperience = Array.isArray(value.workExperience)
        ? value.workExperience.map((entry) => ({
            ...
            skills: Array.isArray(entry?.skills)
                ? entry.skills.filter((skill) => typeof skill === 'string').map((skill) => skill.trim()).filter(Boolean)
                : [],
        }))
        : [];
    ...
    const projects = Array.isArray(value.projects)
        ? value.projects.map((project) => ({
            ...
            kind: project?.kind === 'certification' ? 'certification' : 'project',
...
export function buildHydratedResumeData(parsed) {
