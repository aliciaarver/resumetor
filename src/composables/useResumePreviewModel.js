...
export function useResumePreviewModel() {
    ...
    function formatDateRange(from, to, isCurrent) {
    ...
    function buildLinkHref(link) {
        return buildSocialLinkHref(link);
    }
    function formatSocialLinkLabel(label) {
        return localeStore.socialLinkLabel(label);
    }
    const contactLinks = computed(() => data.value.personal.links
        .map((link) => {
