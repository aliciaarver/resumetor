...
export interface ParseDiff {
  totalChanges: number
  blocks: ParseDiffBlock[]
}

export function buildParseDiff(before: ResumeData, after: ResumeData): ParseDiff {
  const blocks: ParseDiffBlock[] = [
...
function diffPersonal(before: ResumeData, after: ResumeData): ParseDiffBlock {
  let changeCount = 0
  let addedCount = 0
  let removedCount = 0
  ...
  if (beforeName !== afterName) {
    changeCount += 1
    if (!beforeName && afterName) addedCount += 1
    if (beforeName && !afterName) removedCount += 1
  }
  ...
  if (beforePhone !== afterPhone) {
    changeCount += 1
    if (!beforePhone && afterPhone) addedCount += 1
    if (beforePhone && !afterPhone) removedCount += 1
  }
  ...
  for (const [beforeValue, afterValue] of personalFields) {
    if (beforeValue !== afterValue) {
      changeCount += 1
      if (!beforeValue && afterValue) addedCount += 1
      if (beforeValue && !afterValue) removedCount += 1
    }
  }
...
function diffTextBlock(key: ResumeBlockKey, before: string, after: string): ParseDiffBlock {
...
function beforeMapCount(values: string[], target: string): number {
