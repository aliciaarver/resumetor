...
interface ParseTextToResumeOptions {
  ...
  featureFlags?: Partial<ParserFeatureFlags>
}

type SectionKey = keyof typeof SECTION_ALIASES
type ParserProfile = 'hh_ru' | 'en_cv' | 'generic'
...
export interface DetailedResumeParseResult {
  resume: Partial<ResumeData>
  blocks: Record<ResumeBlockKey, ParseBlockMetrics>
}
...
function collectExplicitSectionBuckets(lines: string[]): Record<SectionKey, string[]> {
...
function detectParserProfile(lines: string[]): ParserProfile {
...
function extractDates(text: string): { from: string; to: string; isCurrent: boolean } {
...
function parsePersonalInfo(lines: string[], genericLinkLabel: string): ResumeData['personal'] {
...
function preserveStructuredLine(value: string): string {
  return value
    .replace(/\s+$/g, '')
    .replace(/\s+[|¦]+$/g, '')
    .trimEnd()
...
function firstUrl(lines: string[]): string {
  for (const line of lines) {
...
function looksLikeCompanyUrlLine(line: string, company: string): boolean {
...
function detectProbableFullName(lines: string[]): string {
...
function looksLikeTopPersonalMeta(line: string, index: number): boolean {
...
function looksLikeExperienceNoise(line: string): boolean {
...
function looksLikeDurationSummary(line: string): boolean {
  return (
...
function countFilledPersonalFields(personal: ResumeData['personal']): number {
  return [
