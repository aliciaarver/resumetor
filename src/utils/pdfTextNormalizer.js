export function normalizePdfPageText(items, pageWidth) {
    const tokens = items
...
function toPdfToken(item) {
    const text = normalizeTokenText(item.str);
    const x = item.transform[4] ?? 0;
    const y = item.transform[5] ?? 0;
    const height = Math.max(1, Math.abs(item.height ?? 0), Math.abs(item.transform[0] ?? 0), Math.abs(item.transform[3] ?? 0));
...
function finalizeLine(tokens, y, medianHeight) {
...
function orderLines(lines, pageWidth) {
    ...
    const topOfColumns = Math.max(leftLines[0]?.y ?? Number.NEGATIVE_INFINITY, rightLines[0]?.y ?? Number.NEGATIVE_INFINITY);
    const preamble = lines
        .filter((line) => line.width >= pageWidth * 0.72 && line.y > topOfColumns)
        .sort((a, b) => b.y - a.y);
    const used = new Set([...preamble, ...leftLines, ...rightLines]);
...
function isAuxiliaryMetaColumn(candidate, counterpart, pageWidth) {
    if (candidate.length < 2 || counterpart.length < 4)
        return false;
    const medianWidth = median(candidate.map((line) => line.width));
    if (medianWidth > pageWidth * 0.22)
        return false;
...
function isDateLikeMetaLine(text) {
...
function buildParagraphs(lines) {
    ...
    lines.forEach((line, index) => {
        ...
        paragraphs[paragraphs.length - 1] = mergeParagraphLine(paragraphs[paragraphs.length - 1], currentText);
    });
    return paragraphs
        .map(normalizeParagraphText)
        .filter(Boolean);
}
function getVerticalGaps(lines) {
    const gaps = [];
...
function shouldStartParagraph(previousLine, line, medianGap) {
    if (!previousLine)
        return true;
    if (previousLine.isHeading || line.isHeading)
        return true;
    if (previousLine.isBullet || line.isBullet)
        return true;
    const verticalGap = Math.max(0, previousLine.y - line.y);
    const paragraphGapThreshold = Math.max(medianGap * 1.6, Math.max(previousLine.height, line.height) * 1.15);
    if (verticalGap > paragraphGapThreshold)
        return true;
    if (Math.abs(previousLine.xStart - line.xStart) > Math.max(18, line.height * 1.5))
        return true;
    if (isLineLikelyStandalone(previousLine.text) || isLineLikelyStandalone(line.text))
        return true;
    return false;
}
function mergeParagraphLine(paragraph, line) {
...
function normalizeParagraphText(value) {
    ...
    return normalizeDateRanges(repairUrlSpacing(repairEmailSpacing(emailRepaired
        .replace(/([,;:])([^\s])/g, '$1 $2')
        ...
        if (/@/.test(after) || /[A-Za-z0-9._%+-]@/.test(before + after))
            return match;
        return `${d} ${l}`;
    })
...
function isSoftHyphenBreak(previous, next) {
    return /-$/.test(previous) && /^[a-zа-яё]/i.test(next);
}
function shouldAddSpace(previousChar, nextChar, gap, threshold) {
...
function mergeSoftBreak(previous, next) {
    if (isSoftHyphenBreak(previous, next)) {
        return `${previous.slice(0, -1)}${next}`;
    }
    return `${previous} ${next}`.replace(/\s+/g, ' ').trim();
}
function repairEmailSpacing(value) {
    return value.replace(/\b[A-Za-z0-9._%+-]+(?:\s*[._%+-]\s*[A-Za-z0-9]+)*\s*@\s*[A-Za-z0-9-]+(?:\s*\.\s*[A-Za-z0-9-]+)+\b/g, (match) => match.replace(/\s+/g, ''));
}
function repairUrlSpacing(value) {
    return value
        ...
        .replace(/\b(?:https?:\/\/|www\.|github\.com|linkedin\.com|t\.me|telegram\.me)[A-Za-z0-9./_?=%#@:+~-]*(?:\s+[A-Za-z0-9./_?=%#@:+~-]+)*/gi, (match) => match.replace(/\s+/g, ''));
}
...
function isHeadingLine(text, height, medianHeight) {
    const wordCount = text.split(/\s+/).filter(Boolean).length;
    if (!text)
        return false;
    if (/[.!?]$/.test(text))
        return false;
    if (wordCount <= 5 && text.length <= 48)
        return true;
    if (height >= medianHeight * 1.25 && text.length <= 80)
        return true;
    if (text === text.toUpperCase() && wordCount <= 6)
        return true;
    return false;
}
function isLineLikelyStandalone(text) {
    if (!text)
        return false;
    ...
    if (/[:•]$/.test(text))
        return true;
    if (/^(email|phone|linkedin|github|telegram|contacts?)$/i.test(text))
        return true;
    return false;
}
