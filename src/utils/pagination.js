...
export function measurePagedLayout(element) {
    if (!element) {
        return {
            height: A4_PX_HEIGHT,
            pageStarts: [0],
    ...
    const height = Math.max(A4_PX_HEIGHT, element.scrollHeight, element.offsetHeight, element.clientHeight);
    ...
    const scaleY = rootRect.height > 0 && element.offsetHeight > 0
        ...
        : scaleX || 1;
    const blocks = Array.from(element.querySelectorAll('[data-page-block]')).map((block) => {
