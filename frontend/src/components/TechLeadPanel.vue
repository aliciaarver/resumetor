<template>
  <div class="lead-panel">
    <div class="lead-info">
      <div class="lead-icon">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
          ...
          <path d="M16 3.5 L18.5 6 L22 2.5" stroke="#a6e3a1" stroke-width="2"/>
        </svg>
      </div>
      <div>
        <div class="lead-title">Технический лид</div>
        <div class="lead-desc">Анализирует проект, документирует архитектуру и стек, находит технический долг и создаёт задачи в трекере</div>
      </div>
    </div>
    ...
    <div v-if="!repoPath" class="lead-status lead-status-warn">
      Путь к репозиторию не задан — укажи его при входе в приложение.
    </div>

    <div v-if="running" class="lead-status">
    ...
    <div v-if="result" class="lead-result">
...
async function loadLastResult() {
  ...
  const res = await fetch(`/api/tech-lead-result?${params}`, { headers: authHeaders() });
...
async function analyze() {
  ...
  await fetch('/api/analyze-project', {
    method: 'POST',
...
.analyze-btn:hover:not(:disabled) { background: #b4befe; }
